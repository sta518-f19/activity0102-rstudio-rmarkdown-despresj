...
dino_data %>%
  summarize(r = cor(x, y))
```
The shape is like a dinosaur, however going by the correlation coeffecent it makes a downward sloping line. 
### Exercise 3
...
star_data %>%
  summarize(r = cor(x, y))
```
the correlation coefficent is very similar to the dinosaur because the shape is not linearly correlated.
### Exercise 4
...
circle_data %>%
  summarize(r = cor(x, y))
```
The correlation coeffecnet is again similar to the dinosaur.
### Exercise 5
